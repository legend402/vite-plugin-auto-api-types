...
| `debounceDelay` | `number` | `500` | 防抖延迟（ms），避免短时间内多次请求导致频繁写入文件 |

## 🎨 类型生成规则
1. **类型命名**：基于请求 URL 生成，规则如下：
    - 移除域名部分：`https://api.example.com/user` → `user`
    - 特殊字符替换为下划线：`/api/user/list?page=1` → `api_user_list_page_1`
    - 最终生成：`Api_${处理后的URL}` → `Api_api_user_list_page_1`
2. **基础类型映射**：
